...
#include "ui_pushbutton_parameters.h"
#include <QModbusReply>
#include <QDebug>

pushButton_parameters::pushButton_parameters(QModbusClient *modbusClient, QWidget *parent)  :
    QDialog(parent),
    ui(new Ui::pushButton_parameters),
...
pushButton_parameters::~pushButton_parameters()
{
    delete ui;
}

void pushButton_parameters::setupMapping()
{
...
void pushButton_parameters::on_btnRead_clicked()
{
    ...
    for (const auto &config : m_configMappings) {
        ...
        if (auto *reply = m_modbusClient->sendReadRequest(request, 1)) {
            ...
            connect(reply, &QModbusReply::finished, this, &pushButton_parameters::onReadFinished);
        }
    }
}

void pushButton_parameters::onReadFinished()
{
...
void pushButton_parameters::on_btnWrite_clicked()
{
...
void pushButton_parameters::on_pushButton_Repair_Button_Application_clicked()
{
...
// 注册映射关系
void pushButton_parameters::addMapping(int addr, QWidget* w, double factor, bool isSigned,
...
// 统一设置数值
void pushButton_parameters::setWidgetValue(QWidget* w, double val) {
    if (auto* cb = qobject_cast<QComboBox*>(w)) {
...
// 统一获取数值
double pushButton_parameters::getWidgetValue(QWidget* w) {
    if (!w) return 0;
...
void pushButton_parameters::on_pushButton_Repair_Button_Close_clicked()
{
    close();
}

void pushButton_parameters::on_pushButton_Repair_Button_Confirm_clicked()
{
    if (!m_modbusClient) return;
